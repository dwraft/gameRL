...
class TestGameSimulator(unittest.TestCase):
    ...
    def testTrueCountInRange(self):
        env = BlackjackEnvwithTrueCount(10, natural_bonus=True)
        obs = env.reset()
        env.step(2)  # player joins game
        done = False
        while not done:
            ...
            obs, rewards, done, info = env.step(action)
            _, _, _, count, _ = obs
            condition = count >= -20 and count <= 20
            self.assertTrue(condition, "True count must stay in range")
